...
>) => {
  ...
  return (
    ...
    >
      <SliderPrimitive.Track className="bg-oklch(0.967 0.001 286.375) relative h-[4px] w-full grow overflow-hidden rounded-full dark:bg-oklch(0.274 0.006 286.033)">
        <SliderPrimitive.Range className="bg-oklch(0.21 0.006 285.885) absolute h-full dark:bg-oklch(0.92 0.004 286.32)" />
      ...
      >
        <div className="bg-oklch(0.141 0.005 285.823) absolute size-3 rounded-full dark:bg-oklch(0.985 0 0)" />
      </SliderPrimitive.Thumb>
    </SliderPrimitive.Root>
  )
}
...
export const AudioPlayerTime = ({
  ...
  return (
    <span
      {...otherProps}
      className={cn("text-oklch(0.552 0.016 285.938) text-sm tabular-nums dark:text-oklch(0.705 0.015 286.067)", className)}
    >
      {formatTime(time)}
    </span>
  )
...
function Spinner({ className }: SpinnerProps) {
  return (
    <div
      className={cn(
        "border-oklch(0.967 0.001 286.375) border-t-foreground size-3.5 animate-spin rounded-full border-2 dark:border-oklch(0.274 0.006 286.033)",
        className
      )}
      role="status"
      aria-label="Loading"
