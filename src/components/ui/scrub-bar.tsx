...
function ScrubBarTrack({ className, children, ...props }: ScrubBarTrackProps) {
  ...
  return (
    <div
      ...
      className={cn(
        "bg-oklch(0.967 0.001 286.375) relative h-2 w-full grow cursor-pointer touch-none rounded-full transition-none select-none dark:bg-oklch(0.274 0.006 286.033)",
        className
      )}
...
function ScrubBarThumb({ className, children, ...props }: ScrubBarThumbProps) {
  ...
  return (
    <div
      ...
      className={cn(
        "bg-oklch(0.21 0.006 285.885) absolute top-1/2 block h-4 w-4 -translate-x-1/2 -translate-y-1/2 rounded-full transition-colors disabled:pointer-events-none disabled:opacity-50 dark:bg-oklch(0.92 0.004 286.32)",
        className
      )}
      style={{ left: `${progress}%` }}
      {...props}
